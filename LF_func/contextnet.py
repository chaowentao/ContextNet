...
def feature_extraction(sz_input, sz_input2):
    ...
    # change dialtion by cwt 3/23/2023
    layer1 = _make_layer(firstconv, 4, 2, 1, 1)
    layer2 = _make_layer(layer1, 8, 8, 1, 2)
    layer3 = _make_layer(layer2, 16, 2, 1, 4)
    layer4 = _make_layer(layer3, 16, 2, 1, 8)
    ...
    branch5 = AveragePooling2D((32, 32), (32, 32), 'same')(layer4)
    branch5 = convbn(branch5, 4, 1, 1, 1)
    branch5 = Activation('relu')(branch5)
    branch5 = UpSampling2DBilinear(layer4_size)(branch5)

    output_feature = concatenate([
        layer1, layer2, layer3, layer4, branch5, branch4, branch3, branch2,
        branch1
    ])
...
def _getCostVolume_s1_(inputs):
    ...
    disparity_costs = []
    disparity_values = np.linspace(-0.5, 1.6, 22)
    ...
    cost_volume = K.reshape(cost_volume,
                            (shape[0], 22, shape[1], shape[2], 4 * 81))
    return cost_volume


def _getCostVolume_s2_(inputs):
    shape = K.shape(inputs[0])
    disparity_costs = []
    disparity_values = np.linspace(-2, 2, 9)
    for d in disparity_values:
        if d == 0:
            tmp_list = []
            for i in range(len(inputs)):
...
def basic(cost_volume):
    feature = 2 * 75  # 2 * 75  # 96  # 1 * 75  # 2*75
...
def disparityregression_s1(input):
    shape = K.shape(input)
    disparity_values = np.linspace(-0.5, 1.6, 22)
    x = K.constant(disparity_values, shape=[22])
...
def define_contextnet(sz_input, sz_input2, view_n, learning_rate):
    """ 81 inputs"""
    input_list = []
    ...
    # cv2 = Lambda(_getCostVolume_s2_)(wrap_feature_list)
    # """ channel attention """
    ...
    # pred_s2 = Lambda(disparityregression_s2)(pred_score_s2)
    # pred_s2 = pred_s2 + pred_s1
    model = Model(inputs=input_list, outputs=[pred_s1])
...
if __name__ == '__main__':
    ...
    T1 = time.time()
    model = define_contextnet(input_size, input_size, AngualrViews, 0.001)
    T2 = time.time()
